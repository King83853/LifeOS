...
- Pages scroll inside their OWN frame (3.14), not the document: `.page` is
  ...
  `.tabbar` has `height:var(--tb-h)` (since 3.38: 46px + `--tb-pad-b`, the
  bottom safe area minus 10px, min 6px — ~70px on an iPhone vs the original
  ~105px; 3.37's 62px was "a bit too short"; 24px icons, 10px labels), so the tab
