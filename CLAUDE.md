...
- Pages scroll inside their OWN frame (3.14), not the document: `.page` is
  ...
  `.tabbar` has `height:var(--tb-h)` (since 3.39: 48px + `--tb-pad-b`, the
  bottom safe area minus 6px, min 6px — ~76px on an iPhone vs the original
  ~105px; 3.37's 62px was "a bit too short", 3.38's 70px still "a bit
  higher" please; 24px icons, 10px labels), so the tab
